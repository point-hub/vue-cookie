...
function install(app, options) {
  config = _objectSpread2(_objectSpread2({}, defaultOptions), options);
...
function getAll() {
  ...
  for (var i = 0; i < cookies.length; i++) {
    ...
    var cookie = cookies[i].split("="); // Decoding cookie

    cookie[0] = decodeURIComponent(cookie[0]);
    cookie[1] = decodeURIComponent(cookie[1]); // Push cookie into array
...
function set(name, value) {
  ...
  var formattedCookie = encodeURIComponent(name) + "=" + encodeURIComponent(value);

  for (var optionKey in config) {
    formattedCookie += "; " + optionKey;
    var optionValue = config[optionKey];
